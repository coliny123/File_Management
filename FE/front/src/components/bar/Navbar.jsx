...
function Navbar() {
    ...
    return (
        <div className='fixed z-[1000] w-full h-[80px] bg-white flex items-center'>
            <div className='absolute left-[156px] bg-[#F7F6FB] w-[200px] h-[48px] rounded-[18px] flex items-center font-bold pl-5 max-md:invisible'> <div className='mr-3'>코드입력 :</div><input placeholder='입력 후 Enter' className='w-[100px] h-full rounded-[18px] pointer-events-auto bg-[#F7F6FB] outline-none opacity-50' type="text" value={downloadCode} onChange={(e) => setDownloadCode(e.target.value)} onKeyDown={(e) => {if (e.key === 'Enter') {handleDownloadButton()}}} /></div>
